...
function updateSAST() {
    ...
    const utcTime = nowUTC.getTime(); // UTC milliseconds
    const sastTime = utcTime + (2 * 60 * 60 * 1000); // UTC + 2 hours
    currentDateTimeGMT2 = new Date(sastTime); // Set to SAST
    // Log for debugging
    console.log("UTC Time:", nowUTC.toISOString(), "SAST Time:", currentDateTimeGMT2.toISOString());
}
